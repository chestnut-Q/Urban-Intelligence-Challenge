        ...
        with open(traj_path, 'r') as f:
            ...
            for i in range(719):
                ...
                while(index < len(data_list) - 1):
                    car_id = data_list[index]
                    lane_id = data_list[index + 1]
                    if(lane_dict.__contains__(lane_id) == False):
                        lane_dict[lane_id] = []
                        lane_dict[lane_id].append(car_id)
                    elif(car_id not in lane_dict[lane_id]):
                        lane_dict[lane_id].append(car_id)
                    index += 3
            f.close()
        for key in lane_dict:
            lane_dict[key] = len(lane_dict[key])
        my_dict = sorted(lane_dict.items(), key = lambda x:x[1], reverse=True)
        with open("./rank.json", 'w') as f:
            json.dump(my_dict,f)
            f.close()
        my_dict = my_dict[100: 100 + step]
        my_keys = []
        for i in range(step):
            t = my_dict[i]
...
if __name__ == "__main__":
    ...
    for i in range(times):
        ...
        busiest_lanes = my_busiest.get_busylane(8)                           # 获取车流量前step的车道
        print(busiest_lanes)
        i = 0
