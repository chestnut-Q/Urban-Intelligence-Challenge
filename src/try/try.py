...
access_path = r"D:\学习\2021秋\城市认知智能挑战赛\access.txt"
output_path = r"D:\学习\2021秋\城市认知智能挑战赛\输出结果"
        ...
        try:
            with open(lane_path, "r") as f:
                ...
                if len(start_state[1]) < 50:
                    ...
                    threshold = 0.3
                if p > threshold:
                    my_access = random.sample(start_state[1], random.randint(1, int(len(start_state[1])/2)))
                    print('Too many lanes || Reduce lane')
                else:
                    randomroad = random.sample(lanes, random.randint(1, 100))
